...
The following graphs show a time series decomposition of the "birth rate" of new rulings per month across time. We do this quite easily by using the [tidyverts](https://tidyverts.org/) suite of packages developed by Rob Hyndman and company.
...
After this it's just a matter of using the `feasts::autoplot()` function.
...
Following @kleinberg1999, @fowler2008 calculate *authority* and *score* hubs for each ruling. These are the first eigenvectors of the $\mathbf{C}$ and $\mathbf{B}$ respectively.

As @breiger2000 notes: "several stories might be, and are, told about eigenvectors. They are the low-dimensional scaffolds that support higher-dimensional structures." In this case, the story is supposed to be something like this:
...
_Note. It makes sense that SU rulings are heavily represented as "hubs."_
