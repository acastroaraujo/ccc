...
Following @kleinberg1999, @fowler2008 calculate *authority* and *score* hubs for each ruling. These are the first eigenvectors of $\mathbf{C}$ and $\mathbf{B}$ respectively. Ronald Breiger once noted that _"several stories might be, and are, told about eigenvectors. They are the low-dimensional scaffolds that support higher-dimensional structures."_ In this case, the story is supposed to go something like this:
...
_Note. Given the large number of `T` cases, it makes sense that the most "central" C rulings define what a tutela is (`C-543-92`) and further clarify the circumstances under which a tutela can be presented against decisions made by lower courts (C-590-05)._
