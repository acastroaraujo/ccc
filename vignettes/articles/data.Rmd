...
citations |>
  mutate(across(c(from, to), \(x) stringr::str_extract(x, "^[TCSU]+"))) |>
  mutate(type = paste(from, "→", to)) |>
  ...
  theme_sub_panel(
    background = element_rect(fill = "#E6F4FF"),
    grid.major = element_line(color = "white", linewidth = rel(2)),
    grid.minor = element_line(color = "white", linewidth = rel(2))
  ) +
  scale_x_date(
    name = "From Date",
    date_labels = "'%y",
  ...
  labs(title = "Citation Activity", subtitle = "April 1992-2024")
```
*Note. The lines separating the citations into small squares correspond to the End of Year vacation period. Also, if you squint at the column for 2020, you'll be able to see the effects of COVID on the court activity.*
