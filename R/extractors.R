...
extract_citations <- function(texto) {
  ...
  regex1 <- paste0("\\b(C|SU|T) ?(\\p{Pd}| ) ?(\\d{3,4}A?) ?del?( \\d{1,2} de ", mes, " de)?( ", mes, " \\d{1,2} del?)? \\d\\.?\\d(\\d{2})\\b")
  ...
  regex2.1 <- "\\b(C|SU|T) ?(\\p{Pd}| ) ?(\\d{3,4}A?)(?:\\/|-)(\\d{2})\\b"
  regex2.2 <- "\\b(C|SU|T) ?(\\p{Pd}| ) ?(\\d{3,4}A?)\\/\\d{2}(\\d{2})\\b"
  ...
  return(c(out1, out2.1, out2.2, out3))
}
